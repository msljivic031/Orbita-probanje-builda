...
const cascadeMarker='W6B_TYPE_BINDING_CASCADE_REPAIR_V2';
...
console.log(JSON.stringify({state:'W6B_SETTINGS_GENERATOR_REPAIR_APPLIED',owner:'ci/orbita-redesign-wave6b-legend-settings-transform.cjs',repairs,alreadyAdmittedParts:repairs.length===0,physicalOwner:'src/renderer/app/state/useOrganizationRegistryActions.ts',settingsHostOwner:'unique <PodesavanjaScreen host',typeBindingOwnerDiscovery:'scan reconstructed src for unique Window/orbita/updateDemoOrganization declaration owner; no guessed path',rules:['idempotent','bounded effective-date normalization','physical renderer API type owner discovery','clone proven updateDemoOrganization request/return signature with Workforce request type','every AppShell organization JSX sink receives Workforce writer','no optional-contract weakening','no parallel product owner','fail closed on ambiguous owner'],generatedProductBindingChanged:true},null,2));