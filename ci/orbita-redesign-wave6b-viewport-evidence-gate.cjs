...
function assertHumanTruth(runDir,cap,label){
  ...
  if(cap.html){const html=fs.readFileSync(path.join(root,runDir,cap.html),'utf8');if(/1\.\s*1\.\s*1970|1970-01-01/.test(html))throw Error(`${runDir} ${label} exposes internal system-default 1970 sentinel as user-facing history`);}
}
const out=[];
...
console.log(JSON.stringify({state:'PASS',proof:'W6B_PHYSICAL_VERSIONED_LEGEND_VIEWPORT_GATE',humanTruthGuards:['Workforce Settings captures have no active dialog/overlay','required evidence HTML contains no internal 1970 system-default sentinel','current-vs-previous and cross-viewport PNGs are physically distinct'],captures:out},null,2));
