...
const required=[
 ...
 '<span>Dokument</span><strong>{activeDocument.title}</strong>',
...
console.log(JSON.stringify({state:'PASS',gate:'ORBITA_W7C_DOCUMENTS_RECOVERY_UX',owners:['src/renderer/screens/dokumenti/DokumentiScreen.tsx',path.relative(root,css[0]).replace(/\\/g,'/')],truth:['existing operationState/unlinkReview retained','existing activeDocument/work/link relation truth reused','exact document and Rad visible before unlink commit','relation-only consequence visible','native callbacks unchanged','no renderer file/preview owner']},null,2));
