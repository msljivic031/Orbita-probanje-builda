...
const required=[
 ...
 'data-orbita-w7c-unlink-reason="true"',
 'aria-label="Razlog uklanjanja veze"',
 'data-orbita-w7c-confirm-unlink="true"',
...
for(const unique of ['data-orbita-w7c-relation-review="true"','data-orbita-w7c-unlink-reason="true"','data-orbita-w7c-confirm-unlink="true"','data-orbita-action="documents-confirm-unlink"','data-orbita-action="documents-review-unlink"','data-orbita-action="documents-open-managed"','data-orbita-action="documents-import-native"'])if(count(unique)!==1)throw new Error(`W7C unique owner failure ${unique} count=${count(unique)}`);
...
for(const token of ['.documents-unlink-consequence','grid-template-columns:62px minmax(0,1fr)','overflow-y:auto','scrollbar-gutter:stable','.document-dossier-panel:has(.document-unlink-review)','.document-details-disclosure{display:none}','grid-column:1/-1;grid-row:1','data-orbita-w7c-confirm-unlink="true"'])if(!c.includes(token))throw new Error('W7C CSS invariant missing '+token);
if(!c.includes('.document-unlink-review>div>button:first-child'))throw new Error('W7C cancel action hierarchy missing');
console.log(JSON.stringify({state:'PASS',gate:'ORBITA_W7C_DOCUMENTS_RECOVERY_UX',owners:['src/renderer/screens/dokumenti/DokumentiScreen.tsx',path.relative(root,css[0]).replace(/\\/g,'/')],truth:['canonical DocumentRecord originalFileName reused','existing operationState/unlinkReview retained','existing activeDocument/work/link relation truth reused','exact document and Rad visible before unlink commit','unlink reason has explicit accessible name and stable evidence marker','confirm enabled/disabled proof uses neutral evidence marker rather than action selector','relation-only consequence visible','expanded unlink review remains reachable inside one scrollable dossier owner','recovery consequence is visually before cancel/confirm action row','native callbacks unchanged','no renderer file/preview owner']},null,2));
