...
function identifiers(text){return uniq([...text.matchAll(/\b([A-Za-z_$][\w$]*)\b/g)].map(m=>m[1]).filter(v=>/active|document|link|work|rad|role|item/i.test(v)));}
function boundedActionRegion(id,radius=2200){const at=s.indexOf(id);if(at<0)return'';return s.slice(Math.max(0,at-radius),Math.min(s.length,at+radius));}
