...
if(!s.includes('async function exportPdf()')){
 ...
 const x=`  async function exportPdf() {\n    if (outputBusy) return; setOutputBusy('export'); setOutputStatus('Otvaram izbor lokacije za PDF…');\n    try { const result = await window.orbita?.exportWorkforcePdf(outputRequest); if (!result) throw new Error('Workforce PDF bridge unavailable'); setOutputStatus(result.status === 'cancelled' ? 'Izvoz PDF-a je otkazan.' : \`PDF sačuvan: \${result.fileName}\`); }\n    catch { setOutputStatus('PDF nije sačuvan. Pokušajte ponovo.'); } finally { setOutputBusy(null); }\n  }\n  async function printSheet() {\n    if (outputBusy) return; setOutputBusy('print'); setOutputStatus('Otvaram sistemski dijalog za štampu…');\n    try { const result = await window.orbita?.printWorkforce(outputRequest); if (!result) throw new Error('Workforce print bridge unavailable'); setOutputStatus(result.status === 'submitted' ? 'Dokument je prosleđen sistemu za štampu.' : 'Štampanje nije završeno.'); }\n    catch { setOutputStatus('Štampanje nije završeno. Pokušajte ponovo.'); } finally { setOutputBusy(null); }\n  }\n`;
...
console.log(JSON.stringify({state:'W6C_RENDERER_OUTPUT_BOUND_NOT_ADMITTED',owners:[file,host],truth:['existing Workforce renderer owner reused','typed scope/month/IDs only','organization id forwarded','optional preload bridge fails safely','busy duplicate prevention','aria-live result','file name only, no local path'],notYetClaimed:['type/build','visual','print runtime','W6C pass']},null,2));
