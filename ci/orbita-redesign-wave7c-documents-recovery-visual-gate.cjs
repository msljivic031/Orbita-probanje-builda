...
function manifestDiagnostics(){try{const file=path.join(root,'MANIFEST.json');if(!fs.existsSync(file))return 'manifest=missing';const manifest=JSON.parse(fs.readFileSync(file,'utf8'));const hits=[];const seen=new Set();function visit(node,p='manifest'){if(hits.length>=24||node==null)return;if(typeof node==='string'){if(/documents|unlink|gap|fail/i.test(node))hits.push({path:p,value:node});return;}if(typeof node!=='object'||seen.has(node))return;seen.add(node);if(Array.isArray(node)){node.forEach((v,i)=>visit(v,`${p}[${i}]`));return;}for(const [k,v] of Object.entries(node)){if(/gap|fail|error|scenario/i.test(k)&&typeof v!=='object')hits.push({path:`${p}.${k}`,value:v});visit(v,`${p}.${k}`);}}visit(manifest);return JSON.stringify({summary:manifest.summary||null,diagnosticHits:hits},null,2);}catch(error){return `manifest-diagnostic-error=${error instanceof Error?error.message:String(error)}`;}}
const files=all(root);
function evidence(label){const safe=label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const png=files.find(f=>new RegExp(`${safe}\\.png$`).test(f.replace(/\\/g,'/')));const html=files.find(f=>new RegExp(`${safe}\\.html$`).test(f.replace(/\\/g,'/')));if(!png||!html)throw new Error(`W7C evidence missing ${label}\n${manifestDiagnostics()}`);const size=pngSize(png);if(size.width!==width||size.height!==height)throw new Error(`W7C ${label} viewport ${size.width}x${size.height}`);return {png,html,size,source:fs.readFileSync(html,'utf8')};}
