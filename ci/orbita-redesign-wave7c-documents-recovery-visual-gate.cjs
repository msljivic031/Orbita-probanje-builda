...
for(const token of ['data-orbita-w7c-relation-review="true"','aria-label="Posledica uklanjanja veze"','data-orbita-w7c-unlink-reason="true"','aria-label="Razlog uklanjanja veze"','Potvrdi uklanjanje veze','Dokument i managed fajl ostaju u biblioteci.'])if(!s.includes(token))throw new Error('W7C HTML invariant missing '+token);
...
for(const ev of [invalidEv,readyEv])for(const token of ['data-orbita-w7c-relation-review="true"','data-orbita-w7c-unlink-reason="true"','Potvrdi uklanjanje veze'])if(!ev.source.includes(token))throw new Error(`W7C ${path.basename(ev.html)} missing ${token}`);
const invalidButton=/<button\b([^>]*data-orbita-action="documents-confirm-unlink"[^>]*)>/.exec(invalidEv.source);
const readyButton=/<button\b([^>]*data-orbita-action="documents-confirm-unlink"[^>]*)>/.exec(readyEv.source);
if(!invalidButton||!readyButton)throw new Error('W7C confirm button evidence unresolved');
