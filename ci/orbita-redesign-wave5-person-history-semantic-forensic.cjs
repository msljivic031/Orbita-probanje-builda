...
function namesAndFields(rel){const p=path.join(root,rel);if(!fs.existsSync(p))return {missing:true};const s=fs.readFileSync(p,'utf8');const exports=[...s.matchAll(/export\s+(?:type|interface|class|function|const)\s+([A-Za-z0-9_]+)/g)].map(m=>m[1]);const interfaces=[];for(const m of s.matchAll(/(?:export\s+)?(?:type|interface)\s+([A-Za-z0-9_]+)[^{=]*(?:=\s*)?\{([\s\S]*?)\n\}/g)){const props=[...m[2].matchAll(/^\s*([A-Za-z0-9_]+)\??\s*:/gm)].map(x=>x[1]);interfaces.push({name:m[1],fields:props.slice(0,80)});}const labels=[...s.matchAll(/>([^<>{}\n]{3,80})</g)].map(m=>m[1].trim()).filter(Boolean).filter((x,i,a)=>a.indexOf(x)===i).slice(0,60);return {exports:[...new Set(exports)].slice(0,100),interfaces,uiLabels:labels};}
const knownEvidence=Object.fromEntries(known.map(r=>[r,namesAndFields(r)]));
const history=knownEvidence['src/renderer/screens/ljudi/components/LjudiPersonResponsibilityHistory.tsx']||{};
const verdict={
...
fs.writeFileSync(out,JSON.stringify(result,null,2));console.log(JSON.stringify({verdict,top:inventory.slice(0,20).map(x=>x.file)},null,2));