...
const newMap="legendEntries.slice().sort((a,b)=>a.order-b.order || (a.effectiveFrom ?? '').localeCompare(b.effectiveFrom ?? '')).map((entry) => <span key={entry.versionId ?? `${entry.kind}-${entry.token}-${entry.effectiveFrom ?? entry.source}`} data-kind={entry.kind} title={entry.effectiveFrom ? `Važi od ${new Date(entry.effectiveFrom).toLocaleDateString('sr-RS')}` : undefined}><b>{entry.token}</b>{entry.name}{entry.effectiveFrom ? ` · od ${new Date(entry.effectiveFrom).toLocaleDateString('sr-RS')}` : ''}</span>)";
...
c=c.replace('Legenda se razrešava po efektivnoj verziji za izabrani mesec; arhivirana verzija ne prepisuje istorijsko značenje.','Legenda prikazuje sve efektivne verzije koje su stvarno važile tokom izabranog meseca; istorijsko značenje se ne prepisuje.');
write(component,c);
console.log(JSON.stringify({state:'W6B_MONTH_LEGEND_REPAIR_APPLIED',owners:[legend,monthly,component],truth:['daily cells already resolve as-of date','visible monthly legend now includes every effective version used anywhere in selected month','mid-month rename can show old and new token with effective dates','React keys remain version-stable'],productSemanticsChanged:true,reason:'close real mid-month legend truth gap before visual admission'},null,2));
