...
function blocks(source,selector){const out=[];const esc=x=>x.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const pattern=selector.trim().split(/\s+/).map(esc).join('\\s*');const re=new RegExp(pattern+'\\s*\\{','g');let m;while((m=re.exec(source))){const open=m.index+m[0].lastIndexOf('{');let depth=0,end=-1;for(let i=open;i<source.length;i++){if(source[i]==='{')depth++;else if(source[i]==='}'){depth--;if(depth===0){end=i+1;break}}}if(end<0)throw Error(`unterminated ${selector}`);out.push({at:m.index,open,end,body:source.slice(open+1,end-1)});re.lastIndex=end;}return out}
